...
//					+ "                    left outer join PRODUCT_IMG t4 using (p_no)";
	...
	//구매목록보기
		...
		// 상품 주문하기
		public int OrderFianl(Connection conn, String mId, Timestamp date,String name,
				String email,String phone ,String receiver,String recAddress,String recPhone,
				int priceStr2) {

			int result = 0;
			String sql = "insert into orders (o_no, m_id,o_date,o_name,o_email,o_phone,o_receiver,"
					+ "p_receiver_address,o_receiver_phone,o_total_price) "
					+ "values (SEQUENCE_ORDERS_O_NO.nextval, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
			...
			try {
				...
				pstmt.setString(7, recAddress);
				pstmt.setString(8, recPhone);
				pstmt.setInt(9, priceStr2);
